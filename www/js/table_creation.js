...
function table_creation(counter_minimum, counter_maximum){ 
	...
	for (var counter_row = counter_minimum; counter_row < counter_maximum; counter_row++){ 
		var moreInfo = ("./members.html?m_id=" + memberObj[counter_row].m_id); /* The moreInfo variable creates a custom link refering to the m_id of the chosen person within the table. Clicking this link opens the members.html page which displays all information about the person, which is not shown in the overall table. */ 
        var row = document.createElement("tr"); /* Creates the <tr> element for row creation dynamically */
        var member_attribute = ["m_id", "nachname", "vorname", "geburtsdatum", "ort", "aktiv", "vereinsposition", "klassifizierung"]; /* The creation of an array which is getting the concerned data sets from the data base and fills the lines with the correct records in the tbody.*/
        /* The for-directive execute that every cell is filled with the correct records, which were defined with the variable member_attribute. Once the maximum of the attributes is rechead (8) the loop is concluded. */ 
        for (var counter_cell = 0; counter_cell < member_attribute.length ; counter_cell++){
            ...
            var cellText = document.createTextNode(memberObj[counter_row][member_attribute[counter_cell]]);
