// -------------------------------------------------- //
// Initialization of Forward Button onclick		  	  //
// -------------------------------------------------- //
...
// -------------------------------------------------- //
// Calculation of the next 15 entries 			  //
// -------------------------------------------------- //
...
function function_forward(load_type){
		// Distignuish between first load "initial" or normal next site load
        ...
        clear_table();
		//creates the "number of pages" without remainder if there are more than 15 entries to be displayed
        if (table_site < data_without_rest){
			...
			// if less than 15 entries, the site only has as many entries as there is data (rows)
            counter_minimum = table_site * 15;
